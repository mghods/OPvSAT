...
numComb     = bytes.decode(u"[\u0024\u058f\u060b\u09f2\u09f3\u09fb\u0af1\u0bf9\u0e3f\u17db\ua838\ufdfc\ufe69\uff04\uffe0\uffe1\uffe5\uffe6\u00a2-\u00a5\u20a0-\u20b9]?\\d+(?:\\.\\d+)+%?".encode('utf-8'))
...
separators  = "(?:--+|―|—|~|–|=)"
decorations = bytes.decode(u"(?:[♫♪]+|[★☆]+|[♥❤♡]+|[\u2639-\u263b]+|[\ue001-\uebbb]+)".encode('utf-8'))
...
bfLeft = bytes.decode(u"(♥|0|[oO]|°|[vV]|\\$|[tT]|[xX]|;|\u0ca0|@|ʘ|•|・|◕|\\^|¬|\\*)".encode('utf-8'))
bfCenter = r"(?:[\.]|[_-]+)"
bfRight = r"\2"
s3 = r"(?:--['\"])"
s4 = r"(?:<|&lt;|>|&gt;)[\._-]+(?:<|&lt;|>|&gt;)"
...
eeLeft = r"[＼\\ƪԄ\(（<>;ヽ\-=~\*]+"
eeRight= bytes.decode(u"[\\-=\\);'\u0022<>ʃ）/／ノﾉ丿╯σっµ~\\*]+".encode('utf-8'))
...
def splitEdgePunct(token):
    token = EdgePunctLeft.sub(r"\1\2 \3", token)
    token = EdgePunctRight.sub(r"\1 \2\3", token)
    return token
...
# "foo   bar " => "foo bar"
def squeezeWhitespace(token):
    return Whitespace.sub(" ", token).strip()
